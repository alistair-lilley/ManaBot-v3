...
class DBProxy(Singleton):
    ...
    '''
    def __init__(self, jsonUrl, databaseDir, localUpdateHash, 
                cardsUrl, urlReplStr, DBIDtype, rulesURL):
        try:
            self.httpSession = aiohttp.ClientSession()
            print("http session online")
        except: 
            print("http session failed to connect...")
    ...
    async def _fetchDatabase(self, databaseUrl):
        try:
    ...
    def _saveDatabase(self, databaseDir, jsonFiles):
        for card in jsonFiles:
            with open(f'{databaseDir}/{JSON_PATH}/'
                      f'{self._simplify(card[NAME])}.json', 'w') as jsonCardF:
    ...
    async def _downloadOneCardImage(self, cardName, cardID):
        try:
            ...
            cardpath = self.databaseDir + "/" + IMAGE_PATH + "/"  \
    ...
    async def _downloadCardImages(self, jsonCards):
        ...
        for card in jsonCards:
            ...
            if self._simplify(card[NAME]) in existingCards:
                continue
            await self._downloadOneCardImage(card[NAME], 
    ...
    async def _updateDB(self):
        ...
        jsonCards = self._splitUpJsonCards(await 
                                           self._fetchDatabase(self.jsonUrl))
    ...
    def _makeRemoteImageURL(self, cardName):
        return re.sub(self.urlReplStr, urllib.parse.quote(cardName), 
                      self.cardsurl)